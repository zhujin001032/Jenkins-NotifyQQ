...
public class NotifyQQ extends Notifier {
     ...
     * QQ失败消息内容
     */
    private final String qqmessagefail;
    /**
     * QQ服务器地址
     */
    ...
    public NotifyQQ(String serverurl, List<QQNumber> qQNumbers, String qqmessage ,String qqmessagefail) {
        this.serverurl = serverurl;
        this.qQNumbers = new ArrayList<QQNumber>(qQNumbers);
        this.qqmessage = qqmessage;
        this.qqmessagefail = qqmessagefail;
    }
	...
	public String getQqmessagefail() {
        return qqmessagefail;
    }
    ...
    public boolean perform(AbstractBuild build, Launcher launcher, BuildListener listener) throws UnsupportedEncodingException {
        ...
        String changes = "";
		 try {
            ...
            logger.println("SCM_CHANGELOG = " + changes);
        } catch (Exception e) {
            logger.println("tokenmacro expand error.");
        }
        String msg = "（本通知由Jenkins系统自动发出，无需回复!）\nProject：";
        ...
        if (build.getResult() == Result.SUCCESS){
            ...
            msg += "Jenkins项目地址:" + jobURL + changes+ qqmessagefail + "\n";
        }else{
			//不发出请求
			return false;
        }
...
//         msg = msg.replaceAll("\\+", "_");
        ...
        for (int i = 0; i < qQNumbers.size(); i++) {
            ...
            String qqName = number.GetUrlString().replaceAll("\\+", "%20");

            send(GenerateMessageURL(qqName, msg));
        }
    ...
    public static final class DescriptorImpl extends
        ...
        public FormValidation doCheckNumber(@QueryParameter String value)
            ...
            if (value.length() <= 0)
                return FormValidation.error("QQ昵称太短！");
            else if (value.length() > 25)
                return FormValidation.error("QQ昵称太长！");
